...
# audupa: Compatibility functions for python2.x and python3
def compat_map(f, lst):
    if running_python3():
        return list(map(f, lst))
    else:
        return map(f, lst)

def compat_unicode(s):
    if running_python3():
        return str(s)
    else:
        return unicode(s)
...
_z3_html_op_to_str = {}
for _k, _v in _z3_op_to_str.items():
    _z3_html_op_to_str[_k] = _v
for _k, _v in _z3_pre_html_op_to_str.items():
    _z3_html_op_to_str[_k] = _v

_z3_html_precedence = {}
for _k, _v in _z3_precedence.items():
    _z3_html_precedence[_k] = _v
for _k, _v in _z3_pre_html_precedence.items():
    _z3_html_precedence[_k] = _v
...
class NAryFormatObject(FormatObject):
    def __init__(self, fs):
        assert all(compat_map(lambda a: isinstance(a, FormatObject), fs))
        self.children = fs
    def children(self):
...
class ComposeFormatObject(NAryFormatObject):
    def is_compose(sef):
        return True
    def as_tuple(self):
        return ('compose', compat_map(lambda a: a.as_tuple(), self.children))
    def space_upto_nl(self):
        r = 0
        ...
        return (r, False)
    def flat(self):
        return compose(compat_map(lambda a: a.flat(), self.children))

class ChoiceFormatObject(NAryFormatObject):
    def is_choice(sef):
        return True
    def as_tuple(self):
        return ('choice', compat_map(lambda a: a.as_tuple(), self.children))
    def space_upto_nl(self):
    ...
    def pp_string(self, f, indent):
        if not self.bounded or self.pos <= self.max_width:
            ...
            if self.bounded and self.pos + sz > self.max_width:
                ...
                self.out.write(compat_unicode(f.string))
...
def insert_line_breaks(s, width):
    ...
    for i in range(sz):
        if w > width and s[i] == ' ':
            ...
            new_str.write(compat_unicode(s[i]))
        w = w + 1
