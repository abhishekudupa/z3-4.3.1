...
# audupa: Python2/3 compat
def compat_reduce(a, b, c = None):
    if running_python3():
        return functools.reduce(a,b,c)
    else:
        return reduce(a,b,c)
...
def _coerce_expr_list(alist, ctx=None):
    if filter(is_expr, alist) == []:
        alist = compat_map(lambda a: _py2expr(a, ctx), alist)
    s = compat_reduce(_coerce_expr_merge, alist, None)
    return compat_map(lambda a: s.cast(a), alist)
...
def MultiPattern(*args):
    ...
    if __debug__:
        ...
        _z3_assert(all(compat_map(is_expr, args)), "Z3 expressions expected")
    ctx = args[0].ctx
...
def _mk_quantifier(is_forall, vs, body, weight=1, qid="", skid="", patterns=[], no_patterns=[]):
    if __debug__:
        ...
        _z3_assert(is_const(vs) or (len(vs) > 0 and all(compat_map(is_const, vs))), 
                   "Invalid bounded variable(s)")
        _z3_assert(all(compat_map(lambda a: is_pattern(a) or is_expr(a), patterns)), 
                   "Z3 patterns expected")
        _z3_assert(all(compat_map(is_expr, no_patterns)), "no patterns are Z3 expressions")
    ctx = body.ctx
    if is_app(vs):
        vs = [vs]
    num_vars = len(vs)
    ...
    for i in range(num_vars):
        ...
        _vs[i] = vs[i].as_ast()
    patterns = compat_map(_to_pattern, patterns)
...
def Exists(vs, body, weight=1, qid="", skid="", patterns=[], no_patterns=[]):
    ...
    f(x!foo!5, y!foo!4) >= x!foo!5
    """
    return _mk_quantifier(False, vs, body, weight, qid, skid, patterns, no_patterns)

def _mk_quantifier_named(vs, body, is_forall):
    ctx = body.ctx
    var_names = [to_symbol(str(v)) for v in vs]
    var_sorts = [v.sort() for v in vs]
    num_vars = len(var_names)
    db_vars = [Var(i, var_sorts[i]) for i in range(0, num_vars)]
    subst_exprs = list(zip(vs, db_vars))
    subst_body = substitute(body, *subst_exprs)
    _sorts = (Sort * num_vars)()
    _decls = (Symbol * num_vars)()
    _pats = (Pattern * 0)()
    for i in range(num_vars):
        _decls[num_vars - i - 1] = var_names[i]
        _sorts[num_vars - i - 1] = var_sorts[i].ast
    if is_forall:
        rval = Z3_mk_forall(ctx.ref(), 0, 0, _pats, num_vars, _sorts, _decls, subst_body.ast)
    else:
        rval = Z3_mk_exists(ctx.ref(), 0, 0, _pats, num_vars, _sorts, _decls, subst_body.ast)
    return QuantifierRef(rval, ctx)

def ForAllNamed(vs, body):
    return _mk_quantifier_named(vs, body, True)

def ExistsNamed(vs, body):
    return _mk_quantifier_named(vs, body, False)
...
class IntNumRef(ArithRef):
    ...
    def as_long(self):
        ...
        return compat_long(self.as_string())
...
class BitVecRef(ExprRef):
    ...
    def __rshift__(self, other):
        ...
        >>> BitVecVal(4, 3)
        4
        >>> str(BitVecVal(4, 3).as_signed_long())
        '-4'
        >>> str(simplify(BitVecVal(4, 3) >> 1).as_signed_long())
        '-2'
        >>> str(simplify(BitVecVal(4, 3) >> 1))
        '6'
        >>> str(simplify(LShR(BitVecVal(4, 3), 1)))
        '2'
        >>> str(simplify(BitVecVal(2, 3) >> 1))
        '1'
        >>> str(simplify(LShR(BitVecVal(2, 3), 1)))
        '1'
        """
        a, b = _coerce_exprs(self, other)
...
class BitVecNumRef(BitVecRef):
    ...
    def as_signed_long(self):
        ...
        >>> str(BitVecVal(4, 3).as_signed_long())
        '-4'
        >>> str(BitVecVal(7, 3).as_signed_long())
        '-1'
        >>> str(BitVecVal(3, 3).as_signed_long())
        '3'
        >>> str(BitVecVal(compat_long(2)**32 - 1, 32).as_signed_long())
        '-1'
        >>> str(BitVecVal(compat_long(2)**64 - 1, 64).as_signed_long())
        '-1'
        """
        sz = compat_long(self.size())
        val = self.as_long()
        if (val >= compat_long(2)**(sz - 1)):
            val = val - compat_long(2)**sz
        if (val < -compat_long(2)**(sz - 1)):
            val = val + compat_long(2)**sz
        return val
...
def Concat(*args):
    ...
    args = _get_args(args)
    if __debug__:
        _z3_assert(all(compat_map(is_bv, args)), "All arguments must be Z3 bit-vector expressions.")
...
class Datatype:
    ...
    def declare_core(self, name, rec_name, *args):
        if __debug__:
            ...
            _z3_assert(all(compat_map(_valid_accessor, args)), "Valid list of accessors expected. An accessor is a pair of the form (String, Datatype|Sort)")
...
def CreateDatatypes(*ds):
    ...
    if __debug__:
        ...
        _z3_assert(all(compat_map(lambda d: isinstance(d, Datatype), ds)), 
                   "Arguments must be Datatypes")
        _z3_assert(all(compat_map(lambda d: d.ctx == ds[0].ctx, ds)), "Context mismatch")
        _z3_assert(all(compat_map(lambda d: d.constructors != [], ds)), 
...
def EnumSort(name, values, ctx=None):
    ...
    if __debug__:
        ...
        _z3_assert(all(compat_map(lambda v: isinstance(v, str), values)), 
...
class Fixedpoint(Z3PPObject):
    ...
    def set_predicate_representation(self, f, *representations):
        ...
        representations = compat_map(to_symbol, representations)
...
def ParOr(*ts, **ks):
    ...
    ts  = compat_map(lambda t: _to_tactic(t, ctx), ts)
    sz  = len(ts)
    _args = (TacticObj * sz)()
    for i in range(sz):
        _args[i] = ts[i].tactic
...
def substitute(t, *m):
    ...
    if __debug__:
        ...
        _z3_assert(all(compat_map(lambda p: isinstance(p, tuple) and 
                                  is_expr(p[0]) and is_expr(p[1]) and 
                                  p[0].sort().eq(p[1].sort()), m)), 
...
def substitute_vars(t, *m):
    ...
    if __debug__:
        _z3_assert(is_expr(t), "Z3 expression expected")
        _z3_assert(all(compat_map(is_expr, m)), 
...
def Sum(*args):
    ...
    args  = _coerce_expr_list(args, ctx)
    if is_bv(args[0]):
        return compat_reduce(lambda a, b: a + b, args, 0)
    else:
        _args, sz = _to_ast_array(args)
...
def Product(*args):
    ...
    args  = _coerce_expr_list(args, ctx)
    if is_bv(args[0]):
        return compat_reduce(lambda a, b: a * b, args, 1)
    else:
        _args, sz = _to_ast_array(args)
